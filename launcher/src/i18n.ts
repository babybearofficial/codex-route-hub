...
const en = {
  ...
  supportBody: "Star the repository and open my X profile to help more developers find Codex Route Hub. I share product updates and useful Codex tips on X.",
  star: "Star on GitHub",
  ...
  setupTitle: "Set up Codex Route Hub",
  ...
  mcpCatalogRequired: "Start routing in Startup Configuration before connecting the harness. Local runtime and connector checks can run before refreshing the Codex model picker.",
  tunnelId: "Tunnel ID",
  ...
  launchAtLoginBody: "Open this application at login. Start routing from Startup Configuration on first use.",
  keepRunningOnClose: "Keep the application running when the window closes",
  keepRunningOnCloseBody: "Hide the application and retain its current route state. Stop routing from Startup Configuration.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  continue: "继续",
  supportTitle: "开始之前",
  supportBody: "请为仓库 Star，并打开我的 X 主页，帮助更多开发者发现 Codex Route Hub。我会在 X 分享产品更新和实用的 Codex 技巧。",
  ...
  stepInstallBody: "将 ChatGPT Web 模型添加到 Codex，且不替换原生模型目录。安装完成后统一验证并启动路由。启动前配置会被保存，可在「启动配置」中停止并恢复。",
  ...
  restartCodex: "模型列表刷新可能需要重新打开 Codex。请先在「启动配置」确认路由验证通过，并保存当前任务；不要在配置或验证尚未完成时断开 Codex。",
  ...
  mcpCatalogRequired: "请先在「启动配置」启动并验证路由，再连接 Harness。本地运行时和连接器检查可先完成，无需现在断开 Codex 来刷新模型列表。",
  tunnelId: "Tunnel ID",
  ...
  general: "通用",
  launchAtLogin: "登录时启动",
  launchAtLoginBody: "登录系统时打开本应用；首次使用请在「启动配置」中启动路由。",
  keepRunningOnClose: "关闭窗口后在后台运行",
  keepRunningOnCloseBody: "关闭窗口只隐藏应用，保留当前路由状态。停止路由请使用「启动配置」。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  continue: "続ける",
  supportTitle: "開始する前に",
  supportBody: "より多くの開発者に Codex Route Hub を知ってもらうため、リポジトリに Star を付け、私の X プロフィールを開いてください。X では製品のアップデートや便利な Codex の使い方を紹介しています。",
  star: "GitHub で Star",
  ...
  setupTitle: "Codex Route Hub をセットアップ",
  ...
  mcpCatalogRequired: "「启动配置」でルートを開始してからハーネスを接続してください。ローカル実行環境とコネクターは、Codex のモデル一覧を更新する前に検証できます。",
  tunnelId: "Tunnel ID",
  ...
  launchAtLoginBody: "ログイン時にアプリを開きます。初回は「启动配置」でルートを開始してください。",
  keepRunningOnClose: "ウィンドウを閉じてもサーバーを実行",
  keepRunningOnCloseBody: "ウィンドウを隠し、現在のルート状態を維持します。停止するには「启动配置」を使用してください。",
