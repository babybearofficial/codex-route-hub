...
$Repository = if ($env:CODEX_WEB_GPT_REPOSITORY) { $env:CODEX_WEB_GPT_REPOSITORY } else { "babybearofficial/codex-chatgpt-web" }
...
if (-not $Version) { throw "Could not resolve the latest Codex Route Hub release" }
...
$Asset = "codex-route-hub-$Version-win-$Arch.exe"
$BaseUrl = "https://github.com/$Repository/releases/download/v$Version"
$Temp = Join-Path ([System.IO.Path]::GetTempPath()) "codex-route-hub-$([guid]::NewGuid().ToString('N'))"
New-Item -ItemType Directory -Path $Temp | Out-Null
try {
  if (Get-Process -Name "Codex Route Hub" -ErrorAction SilentlyContinue) {
    throw "Quit Codex Route Hub before updating it"
  }
  ...
  $Executable = Join-Path $InstallLocation "Codex Route Hub.exe"
