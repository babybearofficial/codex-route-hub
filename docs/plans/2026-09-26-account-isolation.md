...
- Isolated packaged smoke passed with an empty instance registry, private scratch homes, hidden background launch and live client restarts disabled. Packaged launch modules match this checkout.
- At the user's request, 5.0.11 replaced `/Applications/Codex Route Hub.app`; the 5.0.10 application and the indexed 5.0.8 backup application were removed. Account/profile directories were not removed. Installed source hashes match the new checkout, its running process has no inherited desktop account environment, and Dock contains one Route Hub entry pointing to the canonical installed path.
- Branch `codex/multi-account-isolation` is published to the independent `babybearofficial/codex-route-hub` repository. The legacy checkout and MultiCodex source were not modified by this Hub change.
